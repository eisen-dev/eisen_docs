...
``` command-line
$ curl -u username:password -i http://192.168.233.130:5000/eisen/api/v1.0/agent
...
``` command-line
$ curl -i "https://localhost/repos/vmg/redcarpet/issues?state=closed"
```
...
``` command-line
$ curl -i -u username -d '{"scopes":["public_repo"]}' https://localhost/authorizations
```
...
``` command-line
$ curl -H "Authorization: token OAUTH-TOKEN" https://localhost
```

### OAuth2 Token (sent as a parameter)

``` command-line
$ curl https://localhost/?access_token=OAUTH-TOKEN
```

Read [more about OAuth2](/v1.1/oauth/).  Note that OAuth2 tokens can be [acquired
programmatically](/v1.1/oauth_authorizations/#create-a-new-authorization), for applications that
are not websites.

### OAuth2 Key/Secret

``` command-line
$ curl 'https://localhost/users/whatever?client_id=xxxx&client_secret=yyyy'
```
...
[uri]: https://eisen.com/hannesg/uri_template
...
API](/v1.0/repos/commits/), pagination is based on
SHA1 and not on page number.

    Link: <https://localhost/user/repos?page=3&per_page=100>; rel="next",
      <https://localhost/user/repos?page=50&per_page=100>; rel="last"

_Linebreak is included for readability._

This `Link` response header contains one or more [Hypermedia](/v1.0/#hypermedia) link relations, some of which may require expansion as [URI templates](http://tools.ietf.org/html/rfc6570).
...
>    "documentation_url": "https://developer.eisen.com/v1.0/#rate-limiting"
> }
```

You can also [check your rate limit status](/v1.0/rate_limit) without incurring an
API hit.
<br>
...
``` command-line
$ curl -i 'https://localhost/users/whatever?client_id=xxxx&client_secret=yyyy'
> HTTP/1.1 200 OK
> Date: Mon, 01 Jul 2013 17:27:06 GMT
> Status: 200 OK
...
To protect the quality of service from eisen, additional rate limits may apply to some actions.
...
may result in abuse rate limiting. These rate limits do not apply to Eisen Enterprise installations.

It is not intended for this rate limit to interfere with any legitimate use of
the API. Your normal [rate limits](/v1.0/#rate-limiting) should be the only
...
>   "documentation_url": "https://developer.eisen.com/v1.0#abuse-rate-limits"
> }
```
...
header will be rejected. We request that you use your Eisen username, or the name of your
...
``` command-line
$ curl -iH 'User-Agent: ' https://localhost/meta
...
> Check https://developer.eisen.com for other possible causes.
```
...
$ curl -i https://localhost/user -H 'If-None-Match: "644b5b0155e6404a9cc4bd9d8b1ae730"'
...
$ curl -i https://localhost/user -H "If-Modified-Since: Thu, 05 Jul 2012 15:31:30 GMT"
> HTTP/1.1 304 Not Modified
> Cache-Control: private, max-age=60
> Last-Modified: Thu, 05 Jul 2012 15:31:30 GMT
> Status: 304 Not Modified
...
``` command-line
$ curl -i https://localhost -H "Origin: http://example.com"
HTTP/1.1 302 Found
Access-Control-Allow-Origin: *
Access-Control-Expose-Headers: ETag, Link, X-Eisen-OTP, X-RateLimit-Limit, X-RateLimit-Remaining, X-RateLimit-Reset, X-OAuth-Scopes, X-Accepted-OAuth-Scopes, X-Poll-Interval
...
``` command-line
$ curl -i https://localhost -H "Origin: http://example.com" -X OPTIONS
HTTP/1.1 204 No Content
Access-Control-Allow-Origin: *
Access-Control-Allow-Headers: Authorization, Content-Type, If-Match, If-Modified-Since, If-None-Match, If-Unmodified-Since, X-Eisen-OTP, X-Requested-With
...
>       ["https://localhost?page=2", {"rel": "next"}]
>     ]
>   },
>   "data": {
>     // the data
...
You can write a JavaScript handler to process the callback. Here's a minimal example you can try out:
    ...
    script.src = 'https://localhost?callback=foo';
...
timestamp. An example of this is the [Commits API](/v1.0/git/commits).

These timestamps look something like `2014-02-27T15:05:06+01:00`. Also see
[this example](https://developer.eisen.com/v1.0/git/commits/#example-input) for
...
``` command-line
$ curl -H "Time-Zone: Europe/Amsterdam" -X POST https://localhost/repos/eisen/linguist/contents/new_file.md
```

This means that we generate a timestamp for the moment your API call is made in
the timezone this header defines. For example, the [Contents API](/v1.0/repos/contents/)
...
timezone is updated whenever you browse the Eisen.com website.
